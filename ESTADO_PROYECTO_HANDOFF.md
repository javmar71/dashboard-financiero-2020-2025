...
> **Actualización (2026-09-20 — EVENTO 31, Arquitectura Multi-Tenant con Aislamiento):** el dashboard (`salidas\dashboard\app.py`) expone ahora un **selector dinámico de Entidad** en el sidebar, descubriendo automáticamente los Data Marts de la entidad ancla (raíz `salidas\power_bi\`) y de `almacen_empresas\<empresa>\salidas\power_bi\`. `agente_financiero\fase10_powerbi_data_mart.py` acepta `--empresa <id>` para generar el Data Mart (7 tablas) en el workspace aislado. PoC `empresa_prueba_2025` (datos **SINTÉTICOS**, semilla 2025) ejecutada y aprobada: 7 tablas + `data_mart_resumen.json` escribidos **solo** en `almacen_empresas\empresa_prueba_2025\salidas\power_bi\`; **inmutabilidad de La Previsora verificada por MD5 (46/46 archivos idénticos en `salidas\`)**; AppTest 0 excepciones (2 entidades, 72 métricas, 11 pestañas, conmutación dinámica OK). Detalle: `07_HISTORIAL_DE_CAMBIOS.md` → EVENTO 31.

> **Actualización (2026-09-20 — EVENTO 32, Centro de Control Web UI y ESTADO OPERATIVO 100 % AUTÓNOMO Y MULTI-TENANT):** el dashboard incorpora la pestaña **"Ingesta"** con **Centro de Control**: (1) **Ingesta estructurada por año** — sube documentación y la deposita en `almacen_empresas\{empresa}\Estados Financieros\ESTADOS_FINANCIEROS_{AÑO}\` (ruta expuesta para pasarla al motor, lectura transparente sin cambios de lógica de negocio); (2) **disparador automático del agente** — botón `🚀 Ejecutar Agente Financiero` lanza `fase10_powerbi_data_mart.py --empresa <id>` vía `subprocess.run` con spinner, y al terminar **auto-refresca** el Data Mart de la empresa en el sidebar (queda seleccionada con sus 72 métricas). **Declaración: el sistema queda en ESTADO OPERATIVO 100 % AUTÓNOMO Y MULTI-TENANT — ya NO se requiere interactuar con la línea de comandos (PowerShell/CMD); todo el ciclo subir documentación → generar Data Mart → ver métricas se opera desde la UI.** Verificado: `py_compile` OK, AppTest 0 excepciones (disparo real con `empresa_prueba_2025` → 72 métricas), depósito año 2025 con estructura limpia y carpetas de prueba removidas. Detalle: `07_HISTORIAL_DE_CAMBIOS.md` → EVENTO 32.
...
9. **ESTADO OPERATIVO 100 % AUTÓNOMO Y MULTI-TENANT (2026-09-20, EVENTO 32):** el sistema opera desde la UI del dashboard (`http://localhost:8501` o `https://dashboard-financiero-2020-2025.streamlit.app`): pestaña **Ingesta** → subir documentación (se estructura en `almacen_empresas\{empresa}\Estados Financieros\ESTADOS_FINANCIEROS_{AÑO}\`) → botón **🚀 Ejecutar Agente Financiero** (corre `fase10_powerbi_data_mart.py --empresa <id>`) → auto-refresco del sidebar con la empresa y sus métricas. **No se requiere PowerShell/CMD para la operación diaria** (la acción interna usa `sys.executable`, no comandos que deba teclear el usuario).
