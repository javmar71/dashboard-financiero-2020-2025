...
**Integración del Centro de Control Web UI: Ingesta estructurada por año y disparador automático del agente (instrucción operativa Big Pickle 2026-09-20).**

- **Módulo de ingesta estructurada** (`salidas\dashboard\app.py`, pestaña "Ingesta"): captura de documentación por empresa y año (2020–2025, default 2025) con subida múltiple (PDF / XLSX / XLS / CSV / DOCX / DOC / TXT / JSON). Deposita en la jerarquía obligatoria `almacen_empresas\{empresa}\Estados Financieros\ESTADOS_FINANCIEROS_{AÑO}\`, mostrando la ruta estructurada (`st.code`) para pasarla al motor de procesamiento (lectura transparente, sin cambios de lógica de negocio). Nombre de empresa saneado con regex (`re.sub`, sin path-traversal).
- **Disparador del agente (Centro de Control):** botón `🚀 Ejecutar Agente Financiero` ejecuta `subprocess.run([sys.executable, "agente_financiero\fase10_powerbi_data_mart.py", "--empresa", <id_sanitizado>], cwd=BASE, capture_output=True, timeout=900)` con `st.spinner` durante el procesamiento; captura stdout/stderr y los muestra al terminar. Fallo → `st.error` con el stderr.
- **Auto-refresco de métricas:** al terminar OK, vía la clave `pending_entidad` (fijada **antes** de instanciar el widget del sidebar) + `st.rerun()`, la nueva empresa queda **seleccionada de inmediato** en el sidebar con sus 72 métricas. Fix documentado: asignar `st.session_state["entidad_sel"]` tras instanciar el `selectbox` con esa key lanza `StreamlitWidgetAlreadyInstantiatedError`; una clave no-widget (`pending_entidad`) lo resuelve.
- **Verificación (2026-09-20):** `py_compile` OK; AppTest del dashboard 0 excepciones — botones `['Generar Informe PDF con IA', 'Depositar en el workspace', '🚀 Ejecutar Agente Financiero']`; disparo real con `empresa_prueba_2025` → transición del sidebar `FIDUCIARIA LA PREVISORA S.A.` → `Empresa Prueba 2025` con 72 métricas; test funcional de depósito (año 2025) creó `...\empresa_ingesta_verif\Estados Financieros\ESTADOS_FINANCIEROS_2025\estados_financieros_2025.pdf` con estructura limpia (solo esa carpeta por año); carpetas de prueba eliminadas tras la verificación.
- **Git:** commit `aa2f95e` ("feat: integra disparador del agente financiero en la pestana Ingesta (subprocess fase10 --empresa, spinner, auto-refresco del sidebar)") pusheado a `origin/master` (`4bf1ab7..aa2f95e`). Working tree limpio.